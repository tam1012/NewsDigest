  ...
  for (const message of batch.messages) {
    ...
    try {
      ...
      if (!content) {
        ...
        if (content) {
        ...
        } else if (status?.description && status.description.length > SCRAPER_SETTINGS.content.descriptionFallbackMinChars) {
          ...
          const quality = content.length >= SCRAPER_SETTINGS.content.richDescriptionMinChars ? '✅ rich' : '⚠️ short';
