...
articles.post('/enqueue-scrape', async (c) => {
    ...
    let enqueued = 0;
    for (let i = 0; i < normalArticles.length; i += SCRAPER_SETTINGS.queue.normalSendBatchSize) {
        const batch = normalArticles.slice(i, i + SCRAPER_SETTINGS.queue.normalSendBatchSize);
    ...
    for (let i = 0; i < redditArticles.length; i += SCRAPER_SETTINGS.queue.redditSendBatchSize) {
        const chunk = redditArticles.slice(i, i + SCRAPER_SETTINGS.queue.redditSendBatchSize);
        await c.env.CONTENT_QUEUE.sendBatch(
            chunk.map((a, j) => ({
                body: { articleId: a.id, url: a.url, title: a.title },
                delaySeconds: (i + j) * SCRAPER_SETTINGS.reddit.queueDelaySeconds,
            }))
        );
