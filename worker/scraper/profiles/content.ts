...
function validateStoredProfile(config: any): ScraperProfileConfig | null {
  const contentSelectors = Array.isArray(config?.contentSelectors)
    ? config.contentSelectors.filter((v: unknown) => typeof v === 'string' && v.trim()).slice(0, SCRAPER_SETTINGS.aiProfile.maxContentSelectors)
    : [];
  if (contentSelectors.length === 0) return null;

  const removeSelectors = Array.isArray(config?.removeSelectors)
    ? config.removeSelectors.filter((v: unknown) => typeof v === 'string' && v.trim()).slice(0, SCRAPER_SETTINGS.aiProfile.maxRemoveSelectors)
    : [];
