...
export async function fetchGitHubTrending(source: Source): Promise<ArticleInput[]> {
  const response = await fetch(source.url, {
    ...
    signal: AbortSignal.timeout(SCRAPER_SETTINGS.sourceFetch.githubTrendingTimeoutMs),
  });
  if (!response.ok) {
  ...
  return results.filter(r => r.url && r.title).slice(0, SCRAPER_SETTINGS.sourceFetch.githubTrendingMaxRepos);
}
