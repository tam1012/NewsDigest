...
export function normalizeConfig(input: any): ScraperProfileConfig | null {
  const contentSelectors = normalizeSelectorArray(input?.contentSelectors, SCRAPER_SETTINGS.aiProfile.maxContentSelectors)
    .filter((s) => !isContentSelectorTooGeneric(s));
  const removeSelectors = normalizeSelectorArray(input?.removeSelectors, SCRAPER_SETTINGS.aiProfile.maxRemoveSelectors);
...
export function normalizeListingConfig(input: any): ListingProfileConfig | null {
  const linkSelectors = normalizeSelectorArray(input?.linkSelectors, SCRAPER_SETTINGS.aiProfile.maxListingSelectors)
...
export async function generateScraperProfile(
  ...
  if (!cleanedHtml || cleanedHtml.length < SCRAPER_SETTINGS.aiProfile.minHtmlChars) return null;
