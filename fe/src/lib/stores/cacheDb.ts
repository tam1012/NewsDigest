    ...
    dbPromise = openDB<NewsDigestDB>(DB_NAME, DB_VERSION, {
      upgrade(db) {
        ...
        if (!db.objectStoreNames.contains('readArticles')) {
          db.createObjectStore('readArticles', { keyPath: 'articleId' });
        }
      },
    });
  }
...
export async function getReadArticleRows(): Promise<ReadArticleRow[]> {
  try {
    const db = await getDb();
    return await db.getAll('readArticles');
  } catch (e) {
    console.warn('[cacheDb] getReadArticleRows failed', e);
    return [];
  }
}

export async function markArticleRead(row: ReadArticleRow): Promise<void> {
  try {
    const db = await getDb();
    await db.put('readArticles', row);
  } catch (e) {
    console.warn('[cacheDb] markArticleRead failed', e);
  }
}
