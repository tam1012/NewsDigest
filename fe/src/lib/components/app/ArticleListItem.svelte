<script lang="ts">
  ...
  let {
    ...
    isSelected?: boolean
    /** Title appears dimmer for already-read item */
    isRead?: boolean
...
>
  ...
  <h3
    class="font-serif text-[1.125em] leading-[1.4] mb-2 font-semibold {isRead
      ? 'text-text-secondary/75'
      : 'text-text-main'} group-hover:underline underline-offset-4 transition-all line-clamp-4 wrap-break-word"
  >
    {@html article.title}
  </h3>
  <p
