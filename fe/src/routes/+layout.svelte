<script lang="ts">
  ...
  import OpenGraph from '$lib/components/app/OpenGraph.svelte'
  ...
  onMount(() => {
    ...
    // Skip body overlay scrollbar on iOS (standalone PWA / mobile Safari)
    // to avoid interfering with native rubber-band & momentum scrolling.
...
>
  ...
  {#if mounted && !isOnline}<div
      id="offline-banner"
      class="fixed top-8 left-1/2 md:top-auto md:bottom-0 md:inset-x-0 md:translate-x-0 z-10 flex -translate-x-1/2 items-center justify-center rounded-full overflow-hidden h-7 w-20 md:rounded-none md:w-full text-xs font-medium"
    >
      <span
