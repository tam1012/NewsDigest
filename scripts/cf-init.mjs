...
function ensureAiBackend(env) {
  const hasGemini = !!env.GEMINI_API_KEY;
  const hasGateway = !!(env.AI_GATEWAY_URL && env.AI_GATEWAY_TOKEN);
  if (!hasGemini && !hasGateway) {
    fail(
      'Thieu cau hinh AI backend. Dien mot trong hai:\n' +
      '  Option A (don gian): GEMINI_API_KEY\n' +
      '  Option B (AI Gateway): AI_GATEWAY_URL + AI_GATEWAY_TOKEN'
    );
  }
}
...
// AI backend — Option A: direct Gemini key, Option B: AI Gateway
if (env.GEMINI_API_KEY) upsertSecret('GEMINI_API_KEY', env.GEMINI_API_KEY);
if (env.AI_GATEWAY_URL) upsertSecret('AI_GATEWAY_URL', env.AI_GATEWAY_URL);
if (env.AI_GATEWAY_TOKEN) upsertSecret('AI_GATEWAY_TOKEN', env.AI_GATEWAY_TOKEN);
