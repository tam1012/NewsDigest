...
// .wrangler/tmp/bundle-xzhs4M/checked-fetch.js
...
// .wrangler/tmp/bundle-xzhs4M/strip-cf-connecting-ip-header.js
...
var init_api = __esm({
  "worker/api/index.ts"() {
    ...
    app.post("/api/articles/enrich", async (c) => {
      ...
      function extractHNArticleUrl(description) {
        const match2 = description.match(/Article URL:\s*<a href="([^"]+)"/);
      ...
      for (let i = 0; i < articles.length; i += 5) {
        ...
        const promises = batch.map(async (art) => {
          ...
          if (art.description && extractHNArticleUrl(art.description)) {
            ...
            if (art.description) {
              const cleaned = art.description.replace(/<hr\s*\/?>/gi, "\n---\n").replace(/<p>/gi, "\n").replace(/<[^>]+>/g, "").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&nbsp;/g, " ").replace(/\n---\n[\s\S]*$/, "").trim();
              if (cleaned.length > 50) {
                await c.env.DB.prepare("UPDATE articles SET content = ? WHERE id = ?").bind(cleaned, art.id).run();
          ...
          if (content && content.length > 50) {
            await c.env.DB.prepare("UPDATE articles SET content = ? WHERE id = ?").bind(content, art.id).run();
        ...
        for (const art of articles) {
          ...
          const result = await c.env.DB.prepare(
            `INSERT OR IGNORE INTO articles (id, source_id, url, title, description, published_at)
                 VALUES (?, ?, ?, ?, ?, ?)`
          ).bind(aId, source.id, art.url, art.title, art.description || "", art.published_at || (/* @__PURE__ */ new Date()).toISOString()).run();
        ...
        try {
          ...
          for (const art of articles) {
            ...
            const result = await c.env.DB.prepare(
              `INSERT OR IGNORE INTO articles (id, source_id, url, title, description, published_at)
                     VALUES (?, ?, ?, ?, ?, ?)`
            ).bind(aId, src.id, art.url, art.title, art.description || "", art.published_at || (/* @__PURE__ */ new Date()).toISOString()).run();
            if (result.meta && result.meta.changes > 0)
              insertedCount++;
          }
...
// .wrangler/tmp/bundle-xzhs4M/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-xzhs4M/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
// worker/queue/content-scraper.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_scraper();
async function handleContentQueue(batch, env) {
  console.log(`\u{1F4E5} Processing ${batch.messages.length} articles for content scraping...`);
  ...
  console.log(`\u{1F4E5} Batch complete.`);
}
__name(handleContentQueue, "handleContentQueue");
